...
if 'predicted_char' not in st.session_state:
    st.session_state.predicted_char = ""
...
class Application:
    ...
    def video_stream(self):
        ...
        with col1:
            ...
            camera_feed = st.empty()  # Placeholder for the live camera feed

        with col2:
            st.subheader("Processed Feed")
            processed_feed = st.empty()  # Placeholder for the processed feed
        ...
        while True:
            ...
            # Display the camera feed (replace the previous frame)
            camera_feed.image(frame_rgb, channels="RGB")
            ...
            if hands:
                    ...
                    if handz:
                            ...
                            res=white
                            self.predict(res)
                    processed_feed.image(white, channels="RGB")
    ...
    def predict(self, test_image):
        ...
        if len(self.str.strip())!=0:
            st1=self.str.rfind(" ")
            ed=len(self.str)
            word=self.str[st1:ed]
            self.word=word
        div = self.str or None
        if str(self.current_symbol).lower() in 'abcdefghijkmnopqrstuvwxyz ':
            st.session_state.predicted_char = self.current_symbol
        ...
        predicted_char_placeholder.write(f"Predicted character: {st.session_state.predicted_char}")
...
# Add buttons for controlling the stream
